...
function getStoryVariation(content, storyName) {
  const variationRegex = new RegExp(
    `export\\s+const\\s+${storyName}\\s*=\\s*Template\\.bind\\(\\{\\}\\s*;\\s*${storyName}\\.args\\s*=\\s*{[^}]*variation:\\s*['"]([^'"]+)['"]`,
    's'
  );
...
function generateTestSpec(stories) {
  ...
  test('${testName} at ${viewport.name} viewport', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('${story.componentId}-${
          story.storyName
        }${variationSuffix}-${viewport.name}.png', {
      clip: box,
      timeout: ${timeout},
      maxDiffPixels: 500,
      threshold: 0.4,
